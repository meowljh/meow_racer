...
from stable.common.buffers import DictReplayBuffer
from stable.common.type_aliases import DictReplayBufferSamples
from stable.common.vec_env import VecEnv, VecNormalize
from stable.her.goal_selection_strategy import KEY_TO_GOAL_STRATEGY, GoalSelectionStrategy
...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> DictReplayBufferSamples:  # type: ignore[override]
        ...
        -> 무조건 episode 하나가 끝나야만 buffer에 저장을 하기 때문에 하나의 episode의 최대 time step보다 learning 시작 시점을 크게 지정해 주어야 한다.
