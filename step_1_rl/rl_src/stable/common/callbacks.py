...
from stable.common.evaluation import evaluate_policy
from stable.common.vec_env import DummyVecEnv, VecEnv, sync_envs_normalization

if TYPE_CHECKING:
    from stable.common import base_class
...
class EvalCallback(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths, logged_data = evaluate_policy(
                self.model,
                self.eval_env,
            ...
            if self.log_path is not None:
                ...
                if len(self._is_success_buffer) > 0:
                    ...
                    if logged_data is not None:
                        ...
                        os.makedirs(f"{self.log_path}/success", exist_ok=True)
                        success_paths = glob(f"{self.log_path}/success/*.pkl")
                        idx = len(success_paths)
                        pickle.dump(
                            logged_data, open(f"{self.log_path}/success/{idx+1}.pkl", "wb")
                        )
                np.savez(
                    self.log_path,
