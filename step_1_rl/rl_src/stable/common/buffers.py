...
from stable.common.preprocessing import get_action_dim, get_obs_shape
...
from stable.common.utils import get_device
from stable.common.vec_env import VecNormalize
...
class RolloutBuffer(BaseBuffer):
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        ...
        for step in reversed(range(self.buffer_size)):
            ...
            # -> 실제 보상: r_t + gamma*V(s_t+1)
            # -> Critic의 value 예측: V(s_t)
            ## delta_t가 TD error임
            
            ## A_t = delta_t + gamma*gae_lambda*A_(t+1)
            ## Critic(s_t): 현재 상태에서의 기대 보상
            ## A_t: 실제 보상과 기대 보상의 차이
            ## R_t = 실제 보상 = A_t + Critic(s_t)
        
            delta = self.rewards[step] + self.gamma * next_values * next_non_terminal - self.values[step]
            ## GAE = delta_t + gamma*gae_lambda*GAE
            # N step 동안의 delta_t들에 gae_lambda를 곱한 가중합을 전부 더함
            # 이유는 TD(1)과 TD(0)의 사이의 값들을 모두 적절하게 결합해서 bias와 variance의 균형을 맞춰야 하기 때문이다.
            # TD(0)이면 bias가 0이고 variance가 큰데, TD(1)은 bias가 크고 variance가 작다.
            last_gae_lam = delta + self.gamma * self.gae_lambda * next_non_terminal * last_gae_lam
            self.advantages[step] = last_gae_lam
        # TD(lambda) estimator, see Github PR #375 or "Telescoping in TD(lambda)"
        # in David Silver Lecture 4: https://www.youtube.com/watch?v=PnHCvfgC_ZA
        ## returns == sum of discounted reward
