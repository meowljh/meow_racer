...
from stable.common.preprocessing import get_action_dim
...
class BetaDistribution(Distribution):
    """
    Beta distribution for continuous actions with bounded limits necessary

    :param action_dim:  Dimension of the action space.
    """
    def __init__(self, action_dim: int, name:str='beta'):
        super().__init__()
        self.action_dim = action_dim
        assert name.upper() == "BETA"

    def proba_distribution_net(self, latent_dim: int):
        """
        Create the layers that represent the distribution
        one output will be the alpha (concentration1)
        the other will be the beta (concentration2)
        """
        alpha_net = nn.Linear(latent_dim, self.action_dim)
        beta_net = nn.Linear(latent_dim, self.action_dim)
        nn.init.zeros_(alpha_net.weight);nn.init.zeros_(beta_net.weight)
        ## 나중에 실제 output alpha, beta 처리 할 때에 
        # Softplus activation function 적용 후에
        # 상수 1을 더해서 alpha, beta >= 1을 무조건 만족하도록 해야 함.
        return alpha_net, beta_net


    def proba_distribution(self, alpha: th.Tensor, beta: th.Tensor):
        """
        Create the distribution given its parameters (alpha, beta)
        """
        alpha = F.softplus(alpha) + 1.
        beta = F.softplus(beta) + 1.
        self.distribution = Beta(concentration1=alpha, concentration0=beta)

        return self
    
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        """
        Get the log probabilities of the actions according to the distribution
        """

        log_prob = self.distribution.log_prob(actions)
        return sum_independent_dims(log_prob)
    
    def actions_from_params(self, alpha: th.Tensor, beta: th.Tensor, deterministic:bool=False):
        """
        Returns actions according to the probability distribution
        - get_actions is the inherited function from the Distribution class
        - self.mode() : when deterministic
        - self.sample() : when stochastic
        """
        self.proba_distribution(alpha=alpha, beta=beta)
    ...
    def log_prob_from_params(self, alpha: th.Tensor, beta: th.Tensor) -> tuple[th.Tensor, th.Tensor]:
        ...
        """
        actions = self.actions_from_params(alpha=alpha, beta=beta)
...
def make_proba_distribution(
    ...
    if isinstance(action_space, spaces.Box):
        ...
        elif 'name' in dist_kwargs and dist_kwargs['name'].upper() == "BETA":
