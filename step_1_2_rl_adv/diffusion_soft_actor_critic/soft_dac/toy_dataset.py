...
def load_twodim(num_samples: int, dataset: str, dimension: int = 2):
    ...
    if dataset == "gmm":
        n_components=2
        dist = GaussianMixture(n_components=n_components)
        dist.weights_ = np.array([0.8, 0.2])
        dist.means_ = np.array([[3., 3.], [-3., -3.]])
        dist.covariances_ = [np.eye(dimension) for _ in range(n_components)]
        
        sample = dist.sample(n_samples = num_samples)[0] #(data sample, indices)
