    ...
    def train_losses(self, denoise_fn, x_0, t, noise=None):
        ...
        if self.loss_type == "kl":
            ...
            if self.sampling_dist == 'uniform':
                ...
                sample_xt = torch.rand_like(x_0) * 12 - 6 #Unif[-6,6]
            elif self.sampling_dist == 'Gaussian':
                sample_xt = torch.randn_like(x_0) * 2 #N(0, 4I)
            ...
            energy = 100 * (0.8 * torch.exp(- torch.linalg.norm(tilde_x_0 - 3 * torch.ones_like(x_t), axis=1) ** 2 / 2)
                  + 0.2 * torch.exp(- torch.linalg.norm(tilde_x_0 + 3 * torch.ones_like(x_t), axis=1) ** 2 /2 )) ##energy term 곱하는 부분
