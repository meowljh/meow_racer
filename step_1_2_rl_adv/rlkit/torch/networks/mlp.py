...
class Mlp(PyTorchModule):
    def __init__(
        ...
        for i, next_size in enumerate(hidden_sizes):
            ...
            if self.layer_norm:
                ln = LayerNorm(next_size) #모든 입력 batch에 대해서 각 data sample마다 normalization을 해줌.
...
class ConcatMlp(Mlp):
    ...
    -> The inputs should be (Batch Size, Num Inputs)
    -> If style conditioned: will be using : (Batch Size, obs_dim+action_dim+style_dim)
    -> Else : (Batch Size, obs_dim+action_dim)
    """
    def __init__(self, *args, dim=1, **kwargs):
        super().__init__(*args, **kwargs)
        self.dim = dim
...
class ParallelMlp(nn.Module):
    ...
    def forward(self, x):
        ...
        return flat.view(batch_size, -1, self.num_heads).squeeze()
