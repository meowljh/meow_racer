...
class TanhNormal(Distribution):
    ...
    def _log_prob_from_pre_tanh(self, pre_tanh_value):
        """Implementation of: "Corrected Soft Actor Critic for Continuous Control" (Oct2024)
        Adapted from
        ...
        :return:
        """
        #읿반적인 Normal Distribution에서 tanh 적용 전의 raw action에 대한 log_prob를 계산하고,
        #pi(a|s) = mu(u|s) * |det(da/du)|^-1
        #log(pi(a|s)) = log(mu(u|s)) - log(det(da/du))
        #위와 같은 이유 때문에 tanh를 적용한 후의 action의 log probability의 값을 보정하는 것이 가능함.
    ...
    def log_prob(self, value, pre_tanh_value=None):
        #pre_tanh_value: arctan value
        if pre_tanh_value is None:
            # errors or instability at values near 1
            # value는 이미 output action에 tanh처리 한 결과
            value = torch.clamp(value, -0.999999, 0.999999)
            ##0.5 * (log((1+y) / (1-y))): arctanh(value)
            pre_tanh_value = torch.log(1+value) / 2 \
                            - torch.log(1-value) / 2
