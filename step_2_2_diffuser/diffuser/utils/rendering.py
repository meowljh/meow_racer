...
# import mujoco
# os.add_dll_directory("....... //.mujoco//mujoco210//bin")
os.add_dll_directory(r"C:\Users\7459985\.mujoco\mujoco210\bin".replace("\\", "/"))
# import mujoco_py as mjc ## actually not needed for gymnasium version of the environment ##
import warnings
import pdb
...
def env_map(env_name):
    ...
    elif 'ant' in env_name: ## 이걸 추가를 안해줘서 제대로 ant rendering이 안됬었음 ##
        return 'AntFullObs-v2'
    else:
        return env_name
...
class MuJoCoRenderer:
    ...
    def __init__(self, env):
        ...
        try:
            ...
            # self.viewer = mjc.MjRenderContextOffscreen(self.env.data)
            # self.viewer = self.env.viewer
            # from gym.envs.mujoco.mujoco_rendering import Viewer
            # self.viewer = Viewer(self.env.model, self.env.data)
            from gym.envs.mujoco.mujoco_rendering import RenderContextOffscreen
            self.viewer = RenderContextOffscreen(self.env.model, self.env.data)
            # breakpoint()
        except:
    ...
    def render(self, observation, dim=256, partial=False, qvel=True, render_kwargs=None, conditions=None):
        ...
        self.viewer.render(camera_id=None, segmentation=False)

        # data = self.viewer.read_pixels(*dim, depth=False)
        data = self.viewer.read_pixels(depth=False, segmentation=False)
        data = data[::-1, :, :]
        return data
    ...
    def composite(self, savepath, paths, dim=(1024, 256), **kwargs):
        ...
        render_kwargs = {
            'trackbodyid': 2,
            'distance':  3., # 10,
            'lookat':  np.array((0., 0., 1.15)), # [5, 2, 0.5],
            'elevation':  -20. # 0
        }
        images = []
        for path in paths:
