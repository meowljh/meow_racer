...
def rollout(
    ...
    agent_infos = []
    env_infos = []
    failed = [] ##added for checking if the episode really succeeded of failed because of time out or out-of track
    next_observations = []
    path_length = 0
    agent.reset()
    o = env.reset()
    ...
    if isinstance(o, tuple):
        o = o[0] #gym Env가 env info dict를 같이 return하게 되기 때문에 필요
    if reset_callback:
    ...
    while path_length < max_path_length:
        ...
        env_step_ret = env.step(copy.deepcopy(a))
        ...
        if done:
            break
        o = next_o #update current observation to next observation
...
def deprecated_rollout(
    ...
    while path_length < max_path_length:
        ...
        env_step_ret = env.step(a)
        if len(env_step_ret) == 4:
            next_o, r, d, env_info = env_step_ret
