...
class MdpPathCollector(PathCollector):
    ...
    def collect_new_paths(
        ...
        for _ in loop:
            ...
            path_action = path['actions'] #(length, action_dim)
            path_reward = path['rewards'] #(length, 1)
            
            
            path_len = len(path['actions'])
            for i in range(path_len):
                self._plotter._log_action(action_arr=path_action[i])
                self._plotter._log_reward(reward=path_reward[i])
                self._plotter._update_global_step()
                
                
            if (
                    path_len != max_path_length #충분한 길이의 path (trajectory)가 모이지 않았고
                    and not path['dones'][-1] #Terminate되지 않았고
                    and discard_incomplete_paths #incomplete path를 discard하는 경우
            ):
                break
